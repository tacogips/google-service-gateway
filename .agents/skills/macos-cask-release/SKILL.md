...
```bash
brew tap user/tap
brew install --cask google-service-gateway
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-service-gateway-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/google-service-gateway-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-service-gateway-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/google-service-gateway-${version}-darwin-x64.dmg"
```
...
`../homebrew-tap/Casks/google-service-gateway.rb`.

After reviewing the rendered tap Cask:

```bash
cd ../homebrew-tap
git add Casks/google-service-gateway.rb README.md
git diff --staged --stat
git commit -m "chore: release google-service-gateway <version>"
...
verify `api/cask/google-service-gateway.json` from GitHub
