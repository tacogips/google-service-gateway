...
| --- | --- |
| macOS Apple Silicon | `google-service-gateway-<version>-darwin-arm64.tar.gz` |
| macOS Intel | `google-service-gateway-<version>-darwin-x64.tar.gz` |
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
scripts/render-homebrew-formula.sh "$version" /path/to/homebrew-tap/Formula/google-service-gateway.rb
```
...
verify `api/formula/google-service-gateway.json` from
